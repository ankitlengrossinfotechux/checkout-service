export function processRefund(refund) {
  let result = { approved: false, actions: [] };
  ...
  if (refund.fraudScore > 80) {
    result.actions.push('fraud-review');
  }
  if (refund.amount > 1000) {
    result.actions.push('manager-review');
  }
  if (refund.reason === 'duplicate') {
    result.actions.push('audit-duplicate-charge');
  }
  if (refund.reason === 'damaged' && refund.requiresReturn) {
    result.actions.push('await-return');
  }
  if (refund.reason === 'late_delivery' || refund.customerTier === 'gold') {
    result.actions.push('priority-service-credit');
  }
  if (refund.marketplaceOrder && !refund.marketplaceApproved) {
    result.actions.push('marketplace-approval');
  }
  if (refund.currency !== refund.settlementCurrency) {
    result.actions.push('fx-adjustment');
  }
  if (refund.partial && refund.items.length > 5) {
    result.actions.push('line-item-reconciliation');
  }
  if (refund.taxIncluded) {
    result.actions.push('tax-reversal');
  }
  if (refund.shippingIncluded) {
    result.actions.push('shipping-reversal');
  }
